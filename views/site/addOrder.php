  ...
  <?php $form = ActiveForm::begin([
      ...
      <div class="row">
        ...
        <div class="col-lg-3" style="margin:10px">
          ...
          ->dropDownList([])->label('客户') ?>
        </div>
      </div>

      <div class="row">
        <div class="col-lg-3" style="margin:10px">
          <?= $form->field($model, 'recommandClientType')
          ->dropDownList($clients['recommand_type'])
          ->label('推荐客户类型') ?>
        </div>

        <div class="col-lg-3" style="margin:10px">
          <?= $form->field($model, 'recommandClientProvince')
          ->dropDownList($province)->label('推荐客户省份') ?>
        </div>

        <div class="col-lg-3" style="margin:10px">
          <?= $form->field($model, 'recommandClient')
          ->dropDownList([])->label('推荐客户') ?>
        </div>
      </div>
      ...
      <table class="table table-striped table-hover" cellspacing="0" width="100%" id="product">
        <thead>
          <tr>
            <th>选择</th>
        ...
        <tbody>
          ...
          <?php if($product->product_residue_num>0): ?>
            <tr>
              ...
              <td><?php echo $product->product_residue_num; ?></td>
              <td><?= $form->field($model, 'productPrice[]')->textInput(array('class' => 'col-lg-6 col-lg-offset-3', 'value' => $product->getProduct($product->product_id)['retail_unit_price'], "id" => $product->purchase_detail_id.'-price'))->label('') ?></td>
              <td><?= $form->field($model, 'productNum[]')->textInput(array('class' => 'col-lg-6 col-lg-offset-3', 'value' => 0,  "id" => $product->purchase_detail_id.'-num'))->label('') ?>
                <?= $form->field($model, 'productId[]')->textInput(['class' => 'col-lg-6 col-lg-offset-3', 'label' => '', 'value' => $product->purchase_detail_id, 'type' => 'hidden'])->label(''); ?></td>
              <td><?= $form->field($model, 'productChannelCost[]')->textInput(['class' => 'col-lg-6 col-lg-offset-3', 'label' => '', "id" => 'cost'.$product->purchase_detail_id])->label(''); ?></td>
              <td><?= $form->field($model, 'productProfit[]')->textInput(['class' => 'col-lg-6 col-lg-offset-3', 'label' => '', "id" => 'profit'.$product->purchase_detail_id])->label(''); ?></td>
              <td><?= $form->field($model, 'totalAccounts[]')->textInput(['class' => 'col-lg-6 col-lg-offset-3', 'label' => '', "id" => 'account'.$product->purchase_detail_id])->label(''); ?></td>
              <td><input type="text" name="guidePice" id="<?= 'guidePrice'.$product->purchase_detail_id?>" readOnly='true' class = 'col-lg-6 col-lg-offset-3'></td>
            </tr>
          <?php endif; ?>
          <?php endforeach; ?>
          <?php endforeach; ?>
...
//  getClient($("#addorderform-clienttype").val(), $("#addorderform-clientprovince").val(), "#addorderform-client");
  $("#addorderform-clienttype").bind('click', function() {
    getClient($(this).val(),$("#addorderform-clientprovince").val(),"#addorderform-client");
  });
  $("#addorderform-clientprovince").bind('click', function() {
    getClient($("#addorderform-clienttype").val(),$("#addorderform-clientprovince").val(),"#addorderform-client");
  });

 // getClient($("#addorderform-recommandclienttype").val(),"#addorderform-recommandclient");
  $("#addorderform-recommandclienttype").bind('click', function() {
    getClient($("#addorderform-recommandclienttype").val(),$("#addorderform-recommandclientprovince").val(),"#addorderform-recommandclient");
  });
  $("#addorderform-recommandclientprovince").bind('click', function() {
    getClient($("#addorderform-recommandclienttype").val(),$("#addorderform-recommandclientprovince").val(),"#addorderform-recommandclient");
  });
});
